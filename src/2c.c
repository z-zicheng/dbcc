...
static unsigned fix_start_bit(bool motorola, unsigned int msg_dlc,
                              unsigned start, unsigned siglen) {
  if (motorola)
    start = (8 * (msg_dlc - 1 - (start / 8))) + (start % 8) - (siglen - 1);
  // printf("start: %d, msg_dlc: %d, siglen: %d\n", start, msg_dlc, siglen);
  return start;
}
...
static int signal2deserializer(signal_t *sig, unsigned int msg_dlc,
  ...
  const unsigned start =
      fix_start_bit(motorola, msg_dlc, sig->start_bit, sig->bit_length);
  const unsigned length = sig->bit_length;
  const char *sig_type = determine_unsigned_type(sig->bit_length);
  const uint64_t mask =
      length == 64 ? 0xFFFFFFFFFFFFFFFFuLL : (1uLL << length) - 1uLL;

  if (comment(sig, o, indent) < 0)
    return -1;
  fprintf(o, "%sstart_byte = %u;\n", indent, start / 8);
  fprintf(o, "%sstart_bit = %u;\n", indent, start % 8);

  uint8_t byte_len = 0;
  const char u8[] = "uint8_t";
  const char u16[] = "uint16_t";
  const char u32[] = "uint32_t";
  const char u64[] = "uint64_t";
  if (strcmp(sig_type, u8) == 0) {
    byte_len = 1;
  } else if (strcmp(sig_type, u16) == 0) {
    byte_len = 2;
  } else if (strcmp(sig_type, u32) == 0) {
    byte_len = 4;
  } else if (strcmp(sig_type, u64) == 0) {
    byte_len = 8;
  } else {
    assert(false);
  }
  fprintf(o, "%smemcpy(&x, %c + start_byte, %d);\n", indent,
          motorola ? 'm' : 'i', byte_len + 1);

  if (start % 8)
    fprintf(o, "%sx = (x >> start_bit) & 0x%" PRIx64 ";\n", indent, mask);
  else
    fprintf(o, "%sx = x & 0x%" PRIx64 ";\n", indent, mask);
...
static int signal2serializer(signal_t *sig, unsigned int msg_dlc,
                             const char *msg_name, FILE *o,
                             const char *indent) {
  assert(sig);
  assert(o);
  bool motorola = (sig->endianess == endianess_motorola_e);
  int start = fix_start_bit(motorola, msg_dlc, sig->start_bit, sig->bit_length);
  const char *sig_type = determine_unsigned_type(sig->bit_length);

  uint64_t mask = sig->bit_length == 64 ? 0xFFFFFFFFFFFFFFFFuLL
                                        : (1uLL << sig->bit_length) - 1uLL;

  if (comment(sig, o, indent) < 0) // TODO: need to confirm
    return -1;

  fprintf(o, "\tmemset(temp, 0, dlc + 1);\n");
  if (sig->is_floating) {
    // TODO: need to confirm
  ...
  fprintf(o, "%smemcpy(temp + start_byte, &x, %d);\n", indent, byte_len + 1);
  if (start % 8 > 0) {
    fprintf(o, "%sfor (int8_t cnt = sizeof(%s); cnt > 0; --cnt) {\n", indent,
            sig_type);
    fprintf(o,
            "%s\ttemp[start_byte + cnt] = (temp[start_byte + cnt] << %d) + "
            "(temp[start_byte + cnt - 1] >> %d);\n",
            indent, start % 8, 8 - start % 8);
    fprintf(o, "%s}\n", indent);
    fprintf(o, "%stemp[start_byte] = (temp[start_byte] << %d);\n", indent,
            start % 8);
  }
  fprintf(o, "%sfor (uint8_t cnt = 0; cnt < dlc; ++cnt) {\n", indent);
  fprintf(o, "%s\t%c[cnt] |= temp[cnt];\n", indent, motorola ? 'm' : 'i');
  fprintf(o, "%s}\n\n", indent);
  return 0;
}
...
static int signal2type(signal_t *sig, FILE *o) {
  ...
  if (sig->is_floating) {
    if (length != 32 && length != 64) {
      warning("signal %s is floating point number but has length %u (fix the "
              "dbc file)",
              sig->name, length);
      return -1;
    }
...
static int signal2scaling_encode(const char *msgname, unsigned id,
                                 ...
                                 const char *god, dbc2c_options_t *copts) {
  assert(msgname);
  assert(sig);
  assert(o);
  assert(copts);
  const char *type =
      determine_type(sig->bit_length, sig->is_signed, sig->is_floating);
  if (sig->scaling != 1.0 || sig->offset != 0.0)
    type = "double";
  if (copts->use_id_in_name)
    fprintf(o, "int encode_can_0x%03x_%s(can_obj_%s_t *o, %s in)", id,
            sig->name, god, copts->use_doubles_for_encoding ? "double" : type);
  else
    fprintf(o, "int encode_can_%s(can_obj_%s_t *o, %s in)", sig->name, god,
            copts->use_doubles_for_encoding ? "double" : type);
...
static int signal2scaling_decode(const char *msgname, unsigned id,
  ...
  if (copts->use_id_in_name)
    ...
    fprintf(o, "int decode_can_%s(const can_obj_%s_t *o, %s *out)", sig->name,
            god, copts->use_doubles_for_encoding ? "double" : type);
  if (header)
    return fputs(";\n", o);
  fputs(" {\n", o);
  if (copts->generate_asserts) {
...
static int print_function_name(FILE *out, const char *prefix, const char *name,
  ...
  assert(god);
  assert(postfix);
  // return fprintf(out, "static int %s_%s(can_obj_%s_t *o, %s %s *data, uint8_t
  // signal_len%s)%s", 		prefix, name, god, in ? "const" : "",
  // datatype, 		dlc ? ", dbcc_time_stamp_t time_stamp" : "",
  // postfix);
  return fprintf(out, "static int %s_%s(can_obj_%s_t *o, %s %s *data%s)%s",
                 prefix, name, god, in ? "const" : "", datatype,
                 dlc ? ", dbcc_time_stamp_t time_stamp" : "", postfix);
}

static void make_name(char *newname, size_t maxlen, const char *name,
                      unsigned id, dbc2c_options_t *copts) {
  assert(newname);
  assert(name);
  assert(copts);
  if (copts->use_id_in_name)
...
static signal_t *find_multiplexor(can_msg_t *msg) {
  ...
  for (size_t i = 0; i < msg->signal_count; i++) {
    ...
    if (sig->is_multiplexor) {
      if (multiplexor)
        error("multiple multiplexor values detected (only one per CAN msg is "
              "allowed) for %s",
              msg->name);
      multiplexor = sig;
    }
...
static signal_t *process_signals_and_find_multiplexer(can_msg_t *msg, FILE *c,
  ...
  for (size_t i = 0; i < msg->signal_count; i++) {
    ...
    if ((serialize ? signal2serializer(sig, msg->dlc, name, c, "\t")
                   : signal2deserializer(sig, msg->dlc, name, c, "\t")) < 0)
...
static int multiplexor_switch(can_msg_t *msg, signal_t *multiplexor, FILE *c,
  ...
  for (size_t i = 0; i < msg->signal_count; i++) {
    ...
    for (; j < msg->signal_count &&
      ...
      if ((serialize
               ? signal2serializer(sig, msg->dlc, msg_name, c, "\t\t")
               : signal2deserializer(sig, msg->dlc, msg_name, c, "\t\t")) < 0)
        return -1;
    }
    i = j - 1;
...
static int msg_pack(can_msg_t *msg, FILE *c, const char *name,
  ...
  fprintf(c, "\tuint8_t dlc = %d;\n", msg->dlc);
  if (copts->generate_asserts) {
    fprintf(c, "\tassert(dlc <= 64);\n\n");
    // fprintf(c, "\tassert(dlc == signal_len)");
  }
  ...
  if (motorola_used) {
    ...
    fprintf(c, "\tuint8_t* m = data;\n");
    fprintf(c, "\tmemset(m, 0, %d * sizeof(uint8_t));\n", msg->dlc);
  }
  if (intel_used) {
    // fprintf(c, "\tuint64_t i = 0;\n");
    fprintf(c, "\tuint8_t* i = data;\n");
    fprintf(c, "\tmemset(i, 0, %d * sizeof(uint8_t));\n", msg->dlc);
  }
  fprintf(c, "\tuint8_t temp[%d];\n\n", msg->dlc + 1);
  fprintf(c, "\tuint8_t start_byte = 0; \n");
  fprintf(c, "\tuint8_t start_bit = 0; \n\n");
  if (!message_has_signals) {
    fprintf(c, "\tUNUSED(o);\n\tUNUSED(data);\n");
  }
...
static int msg_unpack(can_msg_t *msg, FILE *c, const char *name,
  ...
  if (message_has_signals)
    fprintf(c, "\tuint64_t x;\n\n");
  if (motorola_used) {
    fprintf(c, "\tuint8_t m[%d];\n", msg->dlc + 1);
    fprintf(c, "\tm[%d] = 0x00;\n", msg->dlc);
    fprintf(c, "\tmemcpy(m, data, %d);\n", msg->dlc);
    fprintf(c, "%s", swap_motorola ? "\treverse_byte_order(m, dlc);\n\n" : "");
  }
  if (intel_used) {
    fprintf(c, "\tuint8_t i[%d];\n", msg->dlc + 1);
    fprintf(c, "\tmemcpy(i, data, %d);\n", msg->dlc);
    fprintf(c, "%s", swap_motorola ? "" : "\treverse_byte_order(i, dlc);\n\n");
  }
  fprintf(c, "\tuint8_t start_byte = 0; \n");
  fprintf(c, "\tuint8_t start_bit = 0; \n\n");
  if (!message_has_signals)
...
static int msg_dlc_check(can_msg_t *msg) {
  ...
  if (find_multiplexor(msg)) // skip multiplexed messages for now
    return 0;                // TODO: need to confirm
  for (size_t i = 0; i < msg->signal_count; i++) {
    signal_t *s = msg->sigs[i];
    used += s->bit_length;
  }
  if (used > bits) {
    warning("Too many signals, not enough bytes (DLC is too low, fix your DBC "
            "file): %s",
            msg->name);
    return -1;
  }
...
static const char *cfunctions =
    "static inline void reverse_byte_order(uint8_t *x, uint8_t dlc) {\n"
    "\tuint8_t temp;\n"
    "\tuint8_t j = 0;\n"
    "\tfor (uint8_t i = 0; i < dlc/2; ++i) {\n"
    "\t\ttemp = x[dlc - i -1];\n"
    "\t\tx[dlc - i - 1] = x[i];\n"
    "\t\tx[i] = temp;\n"
    "\t}\n"
...
static int signal_compare_function(const void *a, const void *b) {
  ...
  if (ap->bit_length < bp->bit_length)
    return 1;
  if (ap->bit_length == bp->bit_length)
    return 0;
  if (ap->bit_length > bp->bit_length)
...
static int switch_function(FILE *c, dbc_t *dbc, char *function, bool unpack,
  ...
  assert(god);
  assert(copts);
  // fprintf(c, "int %s_message(can_obj_%s_t *o, const unsigned long id, %s
  // %sdata%s)", 		function, god, datatype, unpack ? "" : "*", 		dlc
  // ?
  // ", uint8_t dlc, dbcc_time_stamp_t time_stamp" : "");
  fprintf(
      c,
      "int %s_message(can_obj_%s_t *o, const unsigned long id, %s %s *data%s)",
      function, god, unpack ? "const" : "", datatype,
      dlc ? ", dbcc_time_stamp_t time_stamp" : "");
  if (prototype)
    return fprintf(c, ";\n");
  fprintf(c, " {\n");
  if (copts->generate_asserts) {
    fprintf(c, "\tassert(o);\n");
    fprintf(c, "\tassert(id < (1ul << 29)); /* 29-bit CAN ID is largest "
               "possible */\n");
    // if (dlc)
    // 	fprintf(c, "\tassert(dlc <= 64);         /* Maximum of 64 bytes in a
    // CANFD packet */\n");
  }
...
int dbc2c(dbc_t *dbc, FILE *c, FILE *h, const char *name,
  ...
  /* header file (begin) */
  fprintf(h, "/** CAN message encoder/decoder: automatically generated - do "
             "not edit\n");
  if (copts->use_time_stamps)
    fprintf(h, "  * @note  Generated on %s", asctime(timeinfo));

  fprintf(h, "/** Generated by dbcc.\n"
             "  * This project is forked from https://github.com/howerj/dbcc\n"
             "  *\n"
             "  * @author zhou.zicheng (zhou.zicheng@outlook.com)\n"
             "  *\n"
             "  */\n\n"
             "#ifndef %s\n"
             "#define %s\n\n"
  ...
  fprintf(h, "typedef uint32_t dbcc_time_stamp_t; /* Time stamp for message; "
             "you decide on units */\n");
  fprintf(h, "#endif\n\n");
  ...
  fprintf(h, "\tDBCC_SIG_STAT_UNINITIALIZED_E = 0, /* Message never "
             "sent/received */\n");
  fprintf(h, "\tDBCC_SIG_STAT_OK_E            = 1, /* Message ok */\n");
  fprintf(h, "\tDBCC_SIG_STAT_ERROR_E         = 2, /* "
             "Encode/Decode/Timestamp/Any error */\n");
  ...
  if (copts->generate_unpack)
    switch_function(h, dbc, "unpack", true, true, "uint8_t", true, god, copts);

  if (copts->generate_pack)
    // switch_function(h, dbc, "pack", false, true, "uint8_t", false, god,
    // copts);
    switch_function(h, dbc, "pack", false, true, "uint8_t", false, god, copts);

  if (copts->generate_print)
    switch_function_print(h, dbc, true, god, copts); // TODO: need to confirm
  ...
  if (dbc->use_float)
    fprintf(c, "#include <math.h> /* uses macros NAN, INFINITY, signbit, no "
               "need for -lm */\n");
  if (copts->generate_asserts)
  ...
  fputs(cfunctions, c);
  if (copts->generate_print)
    fputs(cfunctions_print_only, c); // TODO: need to confirm

  if (copts->generate_unpack && dbc->use_float)
    fputs(float_unpack, c); // TODO: need to confirm
  if (copts->generate_pack && dbc->use_float)
    fputs(float_pack, c); // TODO: need to confirm
  ...
  if (copts->generate_unpack)
    switch_function(c, dbc, "unpack", true, false, "uint8_t", true, god, copts);
